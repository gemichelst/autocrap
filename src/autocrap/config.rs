...
pub struct MidiSpec {
    ...
    #[serde(default)] // If velocity is missing in JSON, it defaults to None
    pub velocity: Option<u8>, // Optional velocity for NoteOn (0-127). Defaults to 0x7f (127) if None.
}
...
impl MidiSpec {
    ...
    pub fn index(&self, i: u8) -> MidiSpec {
        MidiSpec {
            ...
            velocity: self.velocity, // Copy the velocity setting
        }
    }
}
...
impl Mapping {
    ...
    pub fn index(&self, i: u8) -> Mapping {
        Mapping {
            ...
            midi: self.midi.map(|m| m.index(i)), // Use the updated MidiSpec::index
        }
    }
