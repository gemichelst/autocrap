...
// --- Constants ---
const DEFAULT_MIDI_VELOCITY: u8 = 0x7f; // Default velocity (127) if not specified in config
        ...
        let midi_resp = self.midi.map(|midi| {
            let data = match midi.kind {
                ...
                MidiKind::NoteOnOff => {
                    if new_state {
                        // Send Note On
                        let velocity = midi.velocity.unwrap_or(DEFAULT_MIDI_VELOCITY); // Use configured or default velocity
                        vec![
                            0b10010000 | midi.channel, // Note On status byte + channel
                            midi.num,                   // Note number
                            velocity.min(127)           // Velocity (ensure it's 0-127)
                        ]
                    } else {
                        // Send Note Off
                        vec![
        ...
        if num == self.ctrl_in_lo_num {
            ...
            let midi_resp = self.midi.map(|midi| {
                let data = match midi.kind {
                    ...
                    // NoteOnOff doesn't make sense for an 8-bit absolute value in this simple implementation
                    MidiKind::NoteOnOff => {
                        warn!("NoteOnOff MIDI Kind is not directly supported for EightBit controls in this manner. Consider mapping velocity differently or using CC.");
